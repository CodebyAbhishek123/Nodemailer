...
o	Navigate to the Google API Console.
o	Create a new project or select an existing one.

3.	Enable Gmail API:
o	Go to the Library section.
o	Search for Gmail API and enable it.

5.	Create OAuth2 Credentials:
o	Go to the Credentials section.
o	Click on Create Credentials and choose OAuth 2.0 Client IDs.
o	Set the application type to Web application.
o	Under Authorized redirect URIs, add http://localhost and https://developers.google.com/oauthplayground (or your application’s URL).
o	After creating, you'll get your ClientID and ClientSecret.

Generating the Refresh Token Using OAuth 2.0 Playground
1.	Access OAuth 2.0 Playground:
o	Open the OAuth 2.0 Playground in your web browser.
2.	Configure OAuth 2.0 Playground:
o	In the top-right corner, click on the gear icon (settings).
o	Under OAuth 2.0 endpoints, select Use your own OAuth credentials.
o	Enter your ClientID and ClientSecret obtained from the Google Cloud Console.
o	Set the Access type to Offline to obtain a refresh token.
3.	Select Scopes:
o	In Step 1 on the left panel, select the appropriate scopes for your application. For Gmail, choose:
o	https://mail.google.com/
4.	Authorize APIs:
o	Click on Authorize APIs. You’ll be redirected to a Google login page to authorize the application.
5.	Exchange Authorization Code for Tokens:
o	After authorizing, you'll be redirected back to the OAuth Playground.
o	Click on Exchange authorization code for tokens. This will generate an access token and a refresh token.
6.	Copy Refresh Token:
o	Your refresh token will appear under the response for Step 2. Copy it and use it in your .env file.
...
3. Set Up Nodemailer with OAuth2
